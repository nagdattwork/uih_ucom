...
const ArticlesDocs = () => {
    ...
    let temp=(prevData?.articles_docs?.split(","))
    temp=temp?.filter((file)=>{return file!="" && file!=undefined && file !='undefined'} )
    const [prevArticle,setPrevArticle]=useState(temp?temp:[])
    const [files, setFiles] = useState(prevData?.current_art_docs?[...prevData?.current_art_docs]:[]);
    const[visible,setVisible]=useState(false)
    const [loading, setLoading] = useState(false);
    const [uploadedFiles,setUploadedFiles]=useState([])
    const dispatch=useDispatch()
    useEffect(()=>{
        let articlesdocsData=data.outcomes
        articlesdocsData={
            ...articlesdocsData,
           ... {
            articles_docs:prevArticle.toString(),
            current_art_docs:uploadedFiles.map((data) => {return (data) })
        ...
        dispatch(appendEdits({
            outcomes:articlesdocsData
        }))


    },[uploadedFiles,prevArticle])
    useEffect(() => {
       const t=
       files.filter((ele)=>{
    ...
    const handleDeletePrevFiles =async (filename,indexT) => {
        ...
        setPrevArticle((prevItems) => prevItems.filter((_, index) => index !== indexT));          
